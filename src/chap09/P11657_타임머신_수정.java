...
public class P11657_타임머신_수정 {
//  private static BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
//  private static BufferedWriter bw = new BufferedWriter(new OutputStreamWriter(System.out));
  static int N, M;
  static long distance[];
  static Edge_11657 edges[];
  public static void main(String[] args) throws IOException {
    ...
    for (int i = 0; i < M; i++) { // 간선 리스트에 데이터 저장하기
      ...
      edges[i] = new Edge_11657(start, end, time);
    }
    // 벨만포드 알고리즘 수행
    distance[1] = 0;
    for (int i = 1; i < N; i++) {  //N보다 하나 적은 수만큼 반복
      for (int j = 0; j < M; j++) {
    	  Edge_11657 edge = edges[j];
...
class Edge_11657 { // 간선리스트를 편하게 다루기 위해 클래스로 별도 구현
  int start, end, time;  // 시작점, 도착점, 걸리는 시간
  public Edge_11657(int start, int end, int time) {
    this.start = start;
    this.end = end;
    this.time = time;
  }
