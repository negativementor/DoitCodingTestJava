...
//  private static BufferedWriter bw = new BufferedWriter(new OutputStreamWriter(System.out));
  ...
  public static void main(String[] args) throws IOException {
    ...
    for (int i = 0; i < M; i++) { // 간선 리스트에 데이터 저장하기
      ...
      Edge_1219s[i] = new Edge_1219_2(start, end, price);
    }
    st = new StringTokenizer(br.readLine());
    for (int i = 0; i < N; i++) {
...
class Edge_1219_2 { // 간선리스트를 편하게 다루기 위해 클래스로 별도 구현
  int start, end, price; // 시작점, 도착점, 걸리는 시간
  public Edge_1219_2(int start, int end, int price) {
    this.start = start;
    this.end = end;
    this.price = price;
  }
